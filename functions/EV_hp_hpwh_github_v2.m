...
n1 = 10;                     % number of homes (= number of HPs)
...
v2hEvents(:,i) = sum(v2h>0.1)
    ...
    UnoptHload(:,i)=P/s + sum(p1base,2)/s + sum(phBase,2)/s + sum(p1basehpwh(1:end-1,:),2)/s;
    OptHload(:,i)=P/s + sum(p1,2)/s + sum(ph - atHome.*pd,2)/s + sum(p2,2)/s;
...
% figure()
% timeline = (datetime(2019,1,1,0,0,0):hours(dt):tEnd)';
% plot(timeline(1:end-1),UnoptHload(:),"LineWidth",2); hold on
% plot(timeline(1:end-1),OptHload(:),"LineWidth",2); hold off
% dt = datetime({'01-Jan-2019 00:00:00','06-Jan-2019 00:00:00','11-Jan-2019 00:00:00','16-Jan-2019 00:00:00','21-Jan-2019 00:00:00','26-Jan-2019 00:00:00'});
% xline(dt,'g--','LineWidth',2)
% grid on
% grid  minor
