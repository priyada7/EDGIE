...
q = zeros(K,n1);      % heat pump electric power, kW
p = zeros(K,n1);          % resistance backup electric power, kW

for k=1:K
    qck = (((Tset(k+1,:) - a1.*Tbase2(k,:))./(1-a1) - theta(k))./R - qe(k,:)); % power to exactly track setpoint, kW
    
    qck_condition = qck < eta1(k, :) .* pMax1(k, :);
   

% Case 1: qck_condition is true
q(k, qck_condition) = max(0, min(eta1(k, qck_condition) .* pMax1(k, qck_condition), qck(qck_condition)));
p(k, qck_condition) = q(k, qck_condition) ./ eta1(k, qck_condition);

% Case 2: qck_condition is false
q(k, ~qck_condition) = max(0, min(eta1(k, ~qck_condition) .* pMax1(k, ~qck_condition) + pMaxAux(k, ~qck_condition), qck(~qck_condition)));
p(k, ~qck_condition) = max(q(k, ~qck_condition) ./ eta1(k, ~qck_condition), q(k, ~qck_condition) + (1 - eta1(k, ~qck_condition)) .* pMax1(k, ~qck_condition));
