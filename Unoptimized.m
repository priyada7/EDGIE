clear
addpath '/Users/priyadarshan/Library/CloudStorage/Box-Box/Herricks/Journal 2/Code_Github/Functions'
addpath '/Users/priyadarshan/Library/CloudStorage/Box-Box/Herricks/Journal 2/Code_Github/InputFiles'
tic
...
stateFolders = { 
  ...
  'MN', '/Users/priyadarshan/Library/CloudStorage/Box-Box/Herricks/CommStock/weather_data/Minnesota/';
   };
...
%% electric vehicle parameters
 ...
 n2 =1*round(n1*n2_1vehicle) + 2*round(n1*n2_2vehicle) + 3*(n1-round(n1*n2_0vehicle)-round(n1*n2_1vehicle)-round(n1*n2_2vehicle));
    ...
    [a2,etac,etad,eMinWinter,eMax,pcMax,pdMax,tau,e0Winter,p0,ecWinter,atHome,atWork,onRoad,tw,th,commuteDistanceProfile,sedan_idx,pickup_idx,alphaWinter] = generateEVmodelsWinter(n2,nDays,K,thetaWinter,batteryDeg,dt,t,commuteDistance,percentageSedans);
    [ecSummer,eMinSummer,e0Summer,alphaSummer]  = generateEVmodelsSummer(n2,nDays,K,thetaSummer,batteryDeg,dt,t,percentageSedans,eMax,commuteDistanceProfile,sedan_idx,pickup_idx);
...
if warmupDays > 0
            ...
            HPWHloadWinter=HPWHloadWinter(warmupSteps+1:end,:);
            phBaseWinter = phBaseWinter(warmupSteps+1:end,:);
            HPloadWinter = HPloadWinter(warmupSteps+1:end,:);
            ...
            HPWHloadSummer=HPWHloadSummer(warmupSteps+1:end,:);
            phBaseSummer = phBaseSummer(warmupSteps+1:end,:);
            HPloadSummer = HPloadSummer(warmupSteps+1:end,:);
            ...
            thetaWinter=thetaWinter(warmupSteps+1:end,:);
            solarWinter=solarWinter(warmupSteps+1:end,:);

            thetaSummer=thetaSummer(warmupSteps+1:end,:);
            solarSummer=solarSummer(warmupSteps+1:end,:);
...
winterPeak =  quantile(  sum(waterHeatingLoadWinter,2) + Pwinter + electricPower_detachedWinter + electricPower_attachedWinter,0.99);


%% summer peak
    check=0;
    [electricPower_detached,~,waterHeatingLoad_detached]=summerPeakcalc(thetaSummer,solarSummer,dt,Psummer,n1,weatherTime,R,cap2_detached_summer_all,HPWHloadSummer,homeWithElectricWH,p1basehpwhSummer,floorAreaDetached,check);
    check=1;
    [electricPower_attached,~,~]=summerPeakcalc(thetaSummer,solarSummer,dt,Psummer,n1,weatherTime,R,cap2_attached_summer_all,HPWHloadSummer,homeWithElectricWH,p1basehpwhSummer,floorAreaAttached,check);
    
 summerPeak = quantile(  sum(waterHeatingLoad_detached,2) + Psummer + sum(electricPower_attached(:,1:round(n1*percentageAttached)),2) + sum(electricPower_detached(:,1:n1-round(n1*percentageAttached)),2),0.99);
...
data{stateIdx,30}= mean((sum(onRoad.*(ecWinter./alphaWinter)+onRoad.*(ecSummer./alphaSummer),1))/(2*nDays-2*warmupDays));
data{stateIdx,31}=((sum(onRoad.*(ecWinter./alphaWinter)+onRoad.*(ecSummer./alphaSummer),'all'))/(n1*(2*nDays-2*warmupDays)));
toc
end
...
headers = {
    ...
    'CommercialSummerPeak (MW)','UpgradeReqCommercial (MW)','Commercial Cost','TotaCostCommercial','Commute_perDay_perCar',...
    'Commute_perDay_perHousehold'...
};
...
folderName = 'Unoptimized_1000home';
