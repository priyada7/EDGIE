...
% fprintf('Winter Start Time: %s\n', datestr(tStartWinter));
% fprintf('Winter End Time:   %s\n', datestr(tEndWinter));
% fprintf('Summer Start Time: %s\n', datestr(tStartSummer));
% fprintf('Summer End Time:   %s\n', datestr(tEndSummer));
...
%% electric vehicle parameters
 ...
 [a2,etac,etad,eMinWinter,eMaxWinter,pcMax,pdMax,tau,e0Winter,p0,ecWinter,atHome,atWork,onRoad,tw,th,commuteDistanceProfile,sedan_idx,pickup_idx,alphaWinter,atGrocery,groceryEnergyWinter,atErrands,errandEnergyWinter] = generateEVmodelsWinter(n2,nDays,K,thetaWinter,batteryDeg,dt,t,commuteDistance,percentageSedans,design_temperatureWinter,design_temperatureSummer,warmupDays);
    [ecSummer,eMinSummer,eMaxSummer,e0Summer,alphaSummer,groceryEnergySummer,errandEnergySummer]  = generateEVmodelsSummer(n2,nDays,K,thetaSummer,batteryDeg,dt,t,percentageSedans,commuteDistanceProfile,sedan_idx,pickup_idx);

eMax = max(eMaxWinter,eMaxSummer);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%% baseline simulation %%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[phBaseSummer,pwBaseSummer,eBaseSummer,pdBaseSummer] = evSimulationSummer(K,n2,e0Summer,p0,eMinSummer,eMax,pcMax,a2,tau,etac,etad,dt,ecSummer,onRoad,atHome,atWork,atGrocery,groceryEnergySummer,atErrands,errandEnergySummer);
[phBaseWinter,pwBaseWinter,eBaseWinter,pdBaseWinter] = evSimulationWinter(K,n2,e0Winter,p0,eMinWinter,eMax,pcMax,a2,tau,etac,etad,dt,ecWinter,onRoad,atHome,atWork,atGrocery,groceryEnergyWinter,atErrands,errandEnergyWinter);
...
%% summer peak
    check=0;
    [electricPower_detached,~,waterHeatingLoad_detached]=summerPeakcalc(thetaSummer(warmupSteps+1:end,:),solarSummer(warmupSteps+1:end,:),dt,Psummer(warmupSteps+1:end,:),n1,weatherTime,R_attached,cap2_detached_summer_all(warmupSteps+1:end,:),HPWHloadSummer(warmupSteps+1:end,:),homeWithElectricWH,p1basehpwhSummer(warmupSteps+1:end,:),floorAreaDetached,check);
    check=1;
    [electricPower_attached,~,~]=summerPeakcalc(thetaSummer(warmupSteps+1:end,:),solarSummer(warmupSteps+1:end,:),dt,Psummer(warmupSteps+1:end,:),n1,weatherTime,R_detached,cap2_attached_summer_all(warmupSteps+1:end,:),HPWHloadSummer(warmupSteps+1:end,:),homeWithElectricWH,p1basehpwhSummer(warmupSteps+1:end,:),floorAreaAttached,check);
...
%[U_units2bldg,A_attached_bldg,A_detached_home,cars_attached,cars_detached,ev_start_attached,EV_to_home,size_detached,size_attached,n_buildings,building_to_home,cars_per_building] = EV_Allocation(units_per_building,n1,n2,n1a,n1d,floorArea,ft2m2);
...
if warmupDays > 0
            ...
            atGrocery=atGrocery(warmupSteps+1:end,:);
            atErrands=atErrands(warmupSteps+1:end,:);
...
% 
% idx0 = find(e0Winter < eMinWinter(1,:) | e0Winter > eMax(1,:));
% length(idx0)
...
data{stateIdx,26} = quantile(LoadOptimizedWinter,0.99);
data{stateIdx,27} =  (data{stateIdx, 11}-data{stateIdx,26})*100/data{stateIdx, 11};


fprintf('n1: %f, Reduction %f \n',n1,(data{stateIdx, 11}-data{stateIdx,26})*100/data{stateIdx, 11});
    fprintf('--------------------------------------------\n');
...
EnergyCommercial,EnergyResidential,normPrice,v2hEligible,A_hv,C_bh,D_bv,scenario,atGrocery,groceryEnergySummer,atErrands,errandEnergySummer);
 ...
 data{stateIdx, 12}=quantile(Psummer/s + HPWHloadSummer/s + sum(phBaseSummer,2)/s + HPloadSummer/s,0.99);
     ...
     data{stateIdx,28} = quantile(LoadOptimizedSummer,0.99);
     data{stateIdx,29} =  (data{stateIdx, 12}-data{stateIdx,28})*100/data{stateIdx, 12};

     fprintf('n1: %f, Reduction %f \n',n1,(data{stateIdx, 12}-data{stateIdx,28})*100/data{stateIdx, 12});
fprintf('--------------------------------------------\n');
...
data{stateIdx,30}= quantile(pWorkWinter_future+sum(atWork.*phWinter,2),0.99)/s;

data{stateIdx,31}= quantile(pWorkSummer_future+sum(atWork.*phSummer,2),0.99)/s;

data{stateIdx,32}= (data{stateIdx,21}-data{stateIdx,30})/data{stateIdx,21};
data{stateIdx,33}= (data{stateIdx,22}-data{stateIdx,31})/data{stateIdx,22};
...
headers = {
    ...
    'R_detached', 'R_attached','Design Temp Heating (F)','Design Temp Cooling (F)','TodaysPeak  (MW)',...
    'Electrified Winter peak (MW)','Electrified Summer Peak (MW)','Change in electrified peak (MW)','Today - Future (MW)','Cost',...
    'zone','TotalCost','HousingUnits','headroom','CommercialTodaysPeak (MW)',...
    'CommercialWinterPeak (MW)','CommercialSummerPeak (MW)','UpgradeReqCommercial (MW)','Commercial Cost','TotaCostCommercial',...
    'OptimizedWinterPeak','WinterReduction%','OptimizedSummerPeak','SummerReduction%','OptimizedWinterPeakCommercial(MW)',...
    'OptimizedSummerPeakCommercial(MW)','WinterReductionCommercial%','SummerReductionCommercial%'
};
