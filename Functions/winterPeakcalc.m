function [electricPowerWinter,waterHeatingLoad]=winterPeakcalc(zone,thetaFull,solarFull,dt,fullP,n1,weatherTime,R,pMax1,HPWHloadFullyear,homeWithHP,HPWHeachHouseLoad,floorArea,pMaxAux,homeWithAux,homeWithElectricWH,percentageAt_DT)
...
if homeWithHP==0
    ...
    pMax1 = pMax1(:,1:homeWithHP);

    electricLoadThermalPower = fullP/n1; % thermal power from electrical loads other than heat pump, kW
    bodyThermalPower = gauss(0.1,0.5,K,homeWithHP); % thermal power from body heat, kW
    solarThermalPower = 0.03*gauss(0.9,1.1,K,homeWithHP).*floorAreahp'.*solarFull; % thermal power from sunlight, kW
    ...
    for k=1:K
        ...
        q(k,:) = max(0, min(eta1(k, :) .*pMax1(k, :), qck));
        p(k, :) = q(k, :) ./ eta1(k, :);
...
if homeWithAux==0
    ...
    electricLoadThermalPower = fullP/n1; % thermal power from electrical loads other than heat pump, kW
    bodyThermalPower = gauss(0.1,0.5,K,homeWithAux); % thermal power from body heat, kW
    solarThermalPower = 0.03*gauss(0.9,1.1,K,homeWithAux).*floorAreaaux'.*solarFull; % thermal power from sunlight, kW
