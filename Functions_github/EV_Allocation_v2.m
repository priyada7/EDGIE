...
%   A_attached: sparse mapping for attached (n_buildings ×
%   n_attached_evs,...
...
% % Display building structure
% fprintf('Building structure:\n');
% fprintf('  Number of buildings: %d\n', n_buildings);
% fprintf('  Units per building: min=%d, max=%d, avg=%.1f\n', ...
%     min(accumarray(building_assignment, 1)), ...
%     max(accumarray(building_assignment, 1)), ...
%     n_attached/n_buildings);
% fprintf('  EVs per building: min=%d, max=%d, avg=%.1f, std=%.2f\n', ...
%     min(cars_per_building), max(cars_per_building), ...
%     mean(cars_per_building), std(cars_per_building));
% 
% % Show sample buildings
% fprintf('\nSample building details:\n');
% for b = 1:min(5, n_buildings)
%     units = find(building_assignment == b);
%     fprintf('  Building %d: %d units, %d total EVs (homes %d-%d)\n', ...
%         b, length(units), cars_per_building(b), ...
%         min(building_to_home{b}), max(building_to_home{b}));
%     
%     % Show unit breakdown
%     for j = 1:min(3, length(units))
%         unit_idx = units(j);
%         fprintf('    Unit %d: %.0f sqft, %d EVs\n', ...
%             unit_idx, size_attached(unit_idx), cars_attached(unit_idx));
%     end
%     if length(units) > 3
%         fprintf('    ... and %d more units\n', length(units)-3);
%     end
% end
% 
% ------------------ BUILD EV-TO-HOME/BUILDING MAPPING ------------------
%fprintf('\n--- Building EV-to-Building mapping ---\n');
...
% fprintf('EVs mapped to buildings: %d\n', EV_count - ev_start_attached + 1);
% fprintf('Total EVs mapped: %d out of %d\n', EV_count, n2);
...
%fprintf('\n--- Creating sparse mapping matrices ---\n');

% ---------------------------
% Indices / counts you have:
%   ev_start_attached: first attached EV index
%   EV_count: total number of EVs
%   n1: total #homes (detached + attached)
%   n_detached: #detached homes (homes 1..n_detached are detached, attached are n_detached+1..n1)
%   n_buildings: #attached buildings
%   EV_to_home(ev): for detached EVs -> detached home id (1..n_detached)
%                    for attached EVs -> building id (1..n_buildings)
%   building_to_home{b}: vector of global home indices in building b (subset of n_detached+1..n1)
% ---------------------------
...
% ------------------ SUMMARY ------------------
% fprintf('\n=== ALLOCATION SUMMARY ===\n');
% fprintf('DETACHED HOMES:\n');
% fprintf('  0 EVs: %d (%.1f%%)\n', sum(cars_detached==0), 100*sum(cars_detached==0)/n_detached);
% fprintf('  1 EV:  %d (%.1f%%)\n', sum(cars_detached==1), 100*sum(cars_detached==1)/n_detached);
% fprintf('  2 EVs: %d (%.1f%%)\n', sum(cars_detached==2), 100*sum(cars_detached==2)/n_detached);
% fprintf('  3+ EVs: %d (%.1f%%)\n', sum(cars_detached>=3), 100*sum(cars_detached>=3)/n_detached);
% 
% fprintf('\nATTACHED UNITS:\n');
% fprintf('  0 EVs: %d (%.1f%%)\n', sum(cars_attached==0), 100*sum(cars_attached==0)/n_attached);
% fprintf('  1 EV:  %d (%.1f%%)\n', sum(cars_attached==1), 100*sum(cars_attached==1)/n_attached);
% fprintf('  2 EVs: %d (%.1f%%)\n', sum(cars_attached==2), 100*sum(cars_attached==2)/n_attached);
% fprintf('  3+ EVs: %d (%.1f%%)\n', sum(cars_attached>=3), 100*sum(cars_attached>=3)/n_attached);
% 
% fprintf('\nATTACHED BUILDINGS:\n');
% fprintf('  Total buildings: %d\n', n_buildings);
% fprintf('  EVs per building: min=%d, max=%d, mean=%.2f, std=%.2f\n', ...
%     min(cars_per_building), max(cars_per_building), ...
%     mean(cars_per_building), std(cars_per_building));
end