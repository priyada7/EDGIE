function [phBase,pwBase,eBase,pdBase]=evSimulationWinter(K,n2,e0,p0,eMin,eMax,pcMax,a2,tau,etac,etad,dt,ec,onRoad,atHome,atWork,atGrocery,groceryEnergy,atErrands,errandEnergy)
...
%%
for k=1:K
    hourOfDay = mod(k-1,24);   % gives 0,1,...,23
    ...
    %pck(~atHome(k,:) & ~atWork(k,:) & ~atErrands(k,:)) = 0;
   %pck(~atHome(k,:) ) = 0;
    ...
    % Grocery discharge
    idx_grocery = (atGrocery(k,:) == 1);
    pdBase(k, idx_grocery) = pdBase(k, idx_grocery) + ...
        etad(idx_grocery) .* groceryEnergy(idx_grocery) / dt;

    % Errands discharge
    idx_errand = (atErrands(k,:) == 1);
    pdBase(k, idx_errand) = pdBase(k, idx_errand) + ...
        etad(idx_errand) .* errandEnergy(idx_errand) / dt;
